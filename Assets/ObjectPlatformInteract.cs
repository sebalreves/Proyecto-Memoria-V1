...
public class ObjectPlatformInteract : MonoBehaviour {
    ...
    private void OnTriggerEnter2D(Collider2D other) {
        if (other.gameObject.CompareTag("PlatformInnerCollider")) {
            other.transform.parent.gameObject.GetComponent<GenericPlatform>().ActivatePlatform();
    ...
    private void OnTriggerExit2D(Collider2D other) {
        if (other.gameObject.CompareTag("PlatformInnerCollider")) {
            other.transform.parent.gameObject.GetComponent<GenericPlatform>().DeactivatePlatform();
    ...
    private void OnTriggerStay2D(Collider2D other) {
        if (other.gameObject.CompareTag("PlatformInnerCollider")) {
            ...
            myRb.AddForce((other.transform.position - gameObject.transform.position) * CONST.PLATFORM_INNER_ATRACTION);
        }
    }
}
